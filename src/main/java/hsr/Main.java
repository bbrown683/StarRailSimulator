...
import hsr.core.simulation.Damage;

public class Main {
    public static void main(String[] args) throws Exception {
        // InputStream seeleJson = Main.class.getResourceAsStream("/character/Seele.json");
        InputStream servalJson = Main.class.getResourceAsStream("/character/Serval.json");
        InputStream makeTheWorldClamorJson = Main.class.getResourceAsStream("/lightcone/MakeTheWorldClamor.json");

        ObjectMapper mapper = new ObjectMapper();
        Character.CharacterBuilder servalBuilder = mapper.readValue(servalJson, Character.CharacterBuilder.class);
        LightCone.LightConeBuilder makeTheWorldClamorBuilder = mapper.readValue(makeTheWorldClamorJson, LightCone.LightConeBuilder.class);
        ...
        serval.equipRelic(new Relic(RelicSet.MUSKETEER_OF_WILD_WHEAT, RelicSlot.BODY, new RelicStat(Stat.CRIT_DMG, 34.4f), new RelicStat(Stat.ATK_PERCENT, 3.4f), new RelicStat(Stat.DEF_PERCENT, 3.8f), new RelicStat(Stat.SPD, 3), new RelicStat(Stat.BREAK_EFFECT, 5.1f)));
        ...
        Damage damage = calculator.getDamage(totalAttack, ((PrimaryEffect)serval.getBasicAttack().getEffects().get(0)).getScaling(), 3, lightningDamageBoost, 
